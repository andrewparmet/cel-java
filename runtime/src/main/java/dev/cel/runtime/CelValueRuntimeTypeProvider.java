...
final class CelValueRuntimeTypeProvider implements RuntimeTypeProvider {
  ...
  // Find the underlying ProtoCelValueConverter. This is required because DefaultInterpreter works
  // with resolved protobuf messages directly in the evaluation flow; the runtime needs a
  // ProtoCelValueConverter to wrap raw Messages as SelectableValues. Both
  // BaseProtoMessageValueProvider (abstract base) and ProtoMessageValueProvider (stand-alone
  // implementation) expose one. ProtoMessageValueProvider previously extended
  // BaseProtoMessageValueProvider but was refactored to implement CelValueProvider directly; the
  // check here needs to cover both shapes.
  private static Optional<BaseProtoCelValueConverter> extractProtoCelValueConverter(
    ...
    if (valueProvider instanceof BaseProtoMessageValueProvider) {
      return Optional.of(
          ((BaseProtoMessageValueProvider) valueProvider).protoCelValueConverter());
    }
    if (valueProvider instanceof ProtoMessageValueProvider) {
      CelValueConverter celConverter =
          ((ProtoMessageValueProvider) valueProvider).celValueConverter();
      if (celConverter instanceof BaseProtoCelValueConverter) {
        return Optional.of((BaseProtoCelValueConverter) celConverter);
      }
    }
